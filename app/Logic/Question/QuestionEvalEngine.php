...
final class QuestionEvalEngine
{
    private static $engine = null;

    private $code;
    private $questionId;
    private $calculationType = EngineConstants::AUTOMATIC;

    public static function getInstance(): QuestionEvalEngine
    {
        if (static::$engine === null) {
            static::$engine = new static();
        }
     ...
     * @return mixed
     */
    private function getSavedEvalCount()
    {
        return QuestionEvalRequest::where('question_id', $this->questionId)
            ->where('code', $this->code)
            ->count();
     ...
     * @return bool
     */
    private function calculateAutomatic($questionId, $code): bool
    {
        $this->code = $code;
     ...
     * @return bool
     */
    private function calculateManual($questionId, $code): bool
    {
        $this->code = $code;
        $this->questionId = $questionId;
    ...
    public function setCode($code): QuestionEvalEngine
    {
        $this->code = $code;
        return self::getInstance();
    }

    public function setQuestionId($questionId): QuestionEvalEngine {
        $this->questionId = $questionId;
        return self::getInstance();
    }

    public function setCalculationType($calculationType): QuestionEvalEngine {
    ...
    public function calculate(): bool
    {
        switch ($this->calculationType) {
            case EngineConstants::AUTOMATIC: {
                return $this->calculateAutomatic($this->questionId, $this->code);
            }
            case EngineConstants::MANUAL: {
                return $this->calculateManual($this->questionId, $this->code);
            }
            default: {
                throw new \RuntimeException('Hatalı parametre girişi yapıldı yalnızca auto veya manual girişi yapılabilir');
            }
        }
    }
}